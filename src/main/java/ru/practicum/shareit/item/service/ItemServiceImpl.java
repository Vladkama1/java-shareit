...
public class ItemServiceImpl implements ItemService {
    ...
    @Override
    public List<ItemOutDto> getAllItemsOwner(Long ownerId) {
        isExistUserInDb(ownerId);
        List<ItemOutDto> itemOutDtoList = itemMapper.toListOutDTO(itemRepository.findAllByOwner_IdOrderById(ownerId));
        List<Long> itemIds = itemOutDtoList.stream()
                .map(ItemOutDto::getId)
                .collect(Collectors.toList());
        List<BookingOutDto> bookings = bookingMapper.toListOutDTO(bookingRepository.findAllByItem_IdInAndStatusIsNot(itemIds, REJECTED));
    ...
    public List<ItemDto> searchBy(String text) {
        ...
        return itemRepository.findByDescriptionContainingIgnoreCaseAndAvailableIsTrue(text.toLowerCase())
                .stream()
